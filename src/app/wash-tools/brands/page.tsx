...
function getBrandLabel(tool: WashTool) {
  return tool.brand?.trim() || UNSET_BRAND;
}
...
export default function WashToolBrandsPage() {
      ...
      try {
        ...
        if (!snapshot.empty) {
          const firestoreTools = snapshot.docs
            ...
            .sort((a, b) => getBrandLabel(a).localeCompare(getBrandLabel(b), 'ja'));
  ...
  const groupedTools = useMemo(() => {
    const brandLabels = Array.from(new Set(tools.map(getBrandLabel))).sort((a, b) => {
      if (a === UNSET_BRAND) return 1;
      if (b === UNSET_BRAND) return -1;
      return a.localeCompare(b, 'ja');
    });

    return brandLabels.map((brand) => ({
      brand,
      tools: tools.filter((tool) => getBrandLabel(tool) === brand),
    }));
  }, [tools]);
  ...
  return (
    ...
    >
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
        ...
        <AppHeaderCard
          icon="🏷️"
          englishLabel="Brand View"
          title="メーカー別一覧"
          description="登録した洗車道具をメーカー・ブランドごとに確認"
        />
        ...
        <div style={{ display: 'grid', gap: '18px' }}>
          {groupedTools.length > 0 ? (
            groupedTools.map(({ brand, tools: brandTools }) => (
              <SectionCard key={brand} marginBottom="0">
                <div
                  style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                ...
                >
                  <h2 style={{ fontSize: '19px', margin: 0 }}>{brand}</h2>
                  <span
                    style={{
                      color: '#a1a1aa',
                      fontSize: '13px',
